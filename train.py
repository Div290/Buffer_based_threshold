...
def evaluate(encoder, classifier, data_loader):
    ...
    ne = 0
    cost = 0
    results = []  # Store results for DataFrame
    sample_id = 0  # Track sample ID
    ...
    for (reviews, mask, labels) in data_loader:
        ...
        with torch.no_grad():
            ...
            for i in range(param.num_exits):
                ...
                confidence = prob_vector.max(dim=1)[0]  # Get confidence for each sample
                pred_cls = preds.argmax(dim=1)  # Predicted class

                # Store results for each sample at this exit
                for j in range(len(labels)):
                    results.append({
                        "Sample_ID": sample_id + j,  # Assign unique ID
                        "Exit": i,
                        "Confidence": confidence[j].item(),
                        "Prediction": pred_cls[j].item(),
                        "Label": labels[j].item()
                    })

                if (confidence > 0.95).any():
                    cost += (i + 1)
                    e += 1
                    # break

                if i == param.num_exits - 1:  # Last exit
                    ne += 1
                    cost += param.num_exits

        sample_id += len(labels)  # Increment sample ID count
        loss += criterion(preds, labels).item()
        acc += pred_cls.eq(labels).sum().item()
